...
loop do
  ...
  if capture_part_2_remainder
    ...
    part_2_remainder -= 1 
    break if part_2_remainder < 0 && iteration > PART_1_ITERATIONS
  end
  ...
  # current_rock.to_grid(rock_x, rock_y).each_key do |pos|
  #   grid[pos] = "#"
  # end
  diff = 0
  if rock_y > highest_y
  ...
  spawn_y = highest_y + 1
  #spawn_y = grid.select { |_,v| v == "#" }.keys.max_by(&:first).first + 1
  ...
  if part_2.nil?
    ys_to_check.each do |sy|
      if (0..4).all? { |x| grid.key?([sy, x]) }
        ...
        # elsif capture_part_2_remainder == false
        #   if pattern == part_2_pattern
        #     part_2 += diff
        #   end
        end
      end
  ...
  puts "Iteration: #{iteration}" if iteration % 1000 == 0
  break if iteration == PART_1_ITERATIONS
end
